	...
	$.ajax({
		...
		success:function(data){
			if(data.flag){//删除成功：移除删除行
				...
				location.reload();
				//window.location.href=path+"/homeRoleList";
			}else{
				//alert("对不起，删除供应商【"+obj.attr("proname")+"】失败");
				changeDLGContent("对不起，删除供应商【"+obj.attr("rolename")+"】失败");
...
$(function(){
	$(".viewRole").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/viewRole?roleid="+ obj.attr("roleid");
	});
	
	$(".modifyRole").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/showRole?roleid="+ obj.attr("roleid");
	});
	...
	$(".deleteRole").on("click",function(){
		roleObj = $(this);
		changeDLGContent("你确定要删除供应商【"+roleObj.attr("rolename")+"】吗？");
		openYesOrNoDLG();
	});
